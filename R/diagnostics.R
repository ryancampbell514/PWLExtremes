# gauge.error.2d = function(w.mesh,est.gfun,true.gfun,est.par,true.par){
#   true.vals = apply(cbind(w.mesh,1-w.mesh),1,true.gfun,par=true.par)
#   est.vals = sapply(w.mesh, est.gfun, par = est.par)
#   return(mean(abs(true.vals-est.vals)))
# }

qq.3d<-function(r,w.adj.angles,r0w,shape=3,par,par.locs,quantilefn=qexp,plot=TRUE,...){
  
  # if(dim(w)[2]==2){w<-cbind(w,1-w[,1]-w[,2])}
...
return.level.score = function(r,w,gfun,par,par.locs,tau,n.dims){
  ...
  T.vals = seq(1/(1-tau),1000,length.out=100)+1
