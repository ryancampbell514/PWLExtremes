...
radial.quants.L1.KDE.bv = function(r,w,tau=0.95,bww=0.05,bwr=0.05,
...
quant.score.bv.checkfn = function(smoothness.lvl,w,r,tau=0.95,k.folds=5,method=c("empirical","KDE"),...){
  ...
  score = mean(sapply(1:k.folds,function(K){
    ...
    if(method=="KDE"){
      r0w.eval = KDE.quant.eval.bv(wpts=w.eval,r=r.fitting,w=w.fitting,tau=tau,bww=smoothness.lvl,bwr=0.05,...)
...
quant.score.bv.checkfn.hR = function(smoothness.lvl,w,r,tau=0.95,k.folds=5,method=c("empirical","KDE"),...){
  ...
  score = mean(sapply(1:k.folds,function(K){
    ...
    # fit the quantile estimates
    if(method=="KDE"){
      r0w.eval = KDE.quant.eval.bv(wpts=w.eval,r=r.fitting,w=w.fitting,tau=tau,bww=0.05,bwr=smoothness.lvl,...)
    } else if (method=="empirical"){
      r0w.eval = empiricalQR.2d.v2(r.fitting=r.fitting,w.fitting=w.fitting,w.eval=w.eval,
                                   tau=tau,wqg=smoothness.lvl)$r.tau.wpts
    }
...
KDE.quant.eval.bv = function(wpts,r,w,tau=0.95,bww=0.05,bwr=0.05,ker="Gaussian"){
  # r, w              -> vectors
  # bww, bwr          -> bandwidths affects smoothness / how close you can get to "pointy" r_0(w)
  # n.mesh            -> mesh for wpts
  # ker.pdf, ker.cdf  -> kernel pdf and cdf functions
