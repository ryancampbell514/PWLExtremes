# 2-d model fitting for when w is on the simplex / unit sphere. NOT Cartesian.

nll.pwlin.2d = function(psi, r, r0w, w.adj.angles, locs, norm, marg, pen.norm , pen.adj, fW.fit, joint.fit, pos.par=TRUE, pen.const=0, fixed.shape=TRUE){
  
  if(!fixed.shape){
    shape=psi[1]
    psi=psi[-1]
  }
  
  # ensure we have no negative parameters
  if(pos.par) {
    if (any(psi <= 0)){
      return(1e+11)
    }
  }

  # rate <- sapply(w, gfun, par = psi)
  if(fW.fit & joint.fit){
    # exceedance radii and angles
    rate = pwlin.g.vals.2d(w.adj.angles,psi,locs,norm,marg)
    ll1 <- dgamma(r, shape = shape, rate = rate, log = T)
    ll2 <- log(pgamma(r0w, shape = shape, rate = rate, lower.tail = F))
    G.vol = G.vol.2d(psi,locs,marg=marg)
    llw = -log(2*G.vol) -2*log(rate)
    NLL = -sum(ll1) + sum(ll2) - sum(llw)
  } else if(!fW.fit & !joint.fit){
    # exceedance radii only
    rate = pwlin.g.vals.2d(w.adj.angles,psi,locs,norm,marg)
    ll1 <- dgamma(r, shape = shape, rate = rate, log = T)
    ll2 <- log(pgamma(r0w, shape = shape, rate = rate, lower.tail = F))
    NLL = -sum(ll1) + sum(ll2)
  } else if(fW.fit & !joint.fit){
    # exceedance angles only
    psi = c(1,psi)  # account for redundancy
    rate = pwlin.g.vals.2d(w.adj.angles,psi,locs,norm,marg)
    G.vol = G.vol.2d(psi,locs,marg=marg)
    llw = -log(2*G.vol) -2*log(rate)
    NLL = -sum(llw)
  }
  if(pen.const>0){
    # pen = abs(diff(psi))
    # pen[c(1,length(pen))] = 2*pen[c(1,length(pen))]  # these appear twice in the penalty
    locs.cart = cbind(locs,1-locs)
    scaled.locs.cart = psi*locs.cart
    coplanar.vecs = apply(scaled.locs.cart,2,diff)
    norm.vecs = cbind(coplanar.vecs[,-1],-coplanar.vecs[,-2])
    grad.g = norm.vecs / apply(scaled.locs.cart[-nrow(scaled.locs.cart),] * norm.vecs, 1,sum)  # gradient of g in each region (is constant/linear in each face)
    pen = apply(grad.g,2,diff)
    if(pen.norm=="2"){
      pen = apply(pen,1,function(vec) sum(vec^2))
    } else if(pen.norm=="1"){
      pen = apply(pen,1,function(vec) sum(abs(vec)))
    }
    
    if(pen.adj){
      pen.multiplier = sapply(1:length(pen),function(r.idx){
        w.low=locs[r.idx]
        w.up=locs[r.idx+2]
        wexc.vec = sapply(w.adj.angles,function(lst) lst$w)
        num = sum(wexc.vec>=w.low & wexc.vec<=w.up)
        if(num==0){
          return(1)
        } else {
          return(num)
        }
      })
      pen = pen/pen.multiplier
    }
    
    pen = sum(pen)
    return(NLL+(pen.const*pen))
  } else {
    return(NLL)
  }
}

fit.pwlin.2d = function(r, r0w, w, locs, norm=NULL,marg="pos", pen.norm="2", pen.adj=F, init.val=NULL, fixed.shape=TRUE, fW.fit=FALSE, joint.fit=FALSE,bound=FALSE,pen.const=0,method="BFGS",...){
  # w.adj.angles -> output of "which.adj.angles.2d"
 
  if(is.null(init.val)){
    init.val = rep(1,length(locs))
  }
  w.adj.angles = which.adj.angles.2d(w,locs,norm=NULL,marg=marg)
  if(fixed.shape & !fW.fit & !joint.fit){
    # only fit the conditional radial model
    opt <- optim(nll.pwlin.2d, par = init.val, r = r, 
                 r0w = r0w, w.adj.angles=w.adj.angles, locs=locs, norm=norm, marg=marg,
                 pen.norm=pen.norm, pen.adj=pen.adj,
                 fW.fit=F,joint.fit=F,pen.const=pen.const,fixed.shape=fixed.shape,
                 control=list(maxit=1e6),method=method,...)
    #method="L-BFGS-B",gr=NULL,lower=rep(0.01,length(locs)),upper=rep(10,length(locs))
    opt$fW.par = NULL  # didn't model angles
    opt$init.val = init.val
    opt$shape=2
  } else if(fixed.shape & fW.fit & joint.fit){
    # only fit the radial model and angular model togethr
    opt <- optim(nll.pwlin.2d, par = init.val, r = r, 
                 r0w = r0w, w.adj.angles=w.adj.angles, locs=locs, norm=norm, marg=marg,
                 pen.norm=pen.norm, pen.adj=pen.adj,
                 fW.fit=T,joint.fit=T,pen.const=pen.const,fixed.shape=fixed.shape,
                 control=list(maxit=1e6),method=method,...)
    opt$fW.par = opt$par
    opt$init.val = init.val
    opt$shape=2
  } else if(fixed.shape & fW.fit & !joint.fit){
    # only fit the angular model together, taking into account the redundancy
    init.val = init.val[-1]  # remove redundancy, fix the first parameter to 1.
    opt <- optim(nll.pwlin.2d, par = init.val, r = r, 
                 r0w = r0w, w.adj.angles=w.adj.angles, locs=locs, norm=norm, marg=marg,
                 pen.norm=pen.norm, pen.adj=pen.adj,
                 fW.fit=T,joint.fit=F,pen.const=pen.const,fixed.shape=fixed.shape,
                 control=list(maxit=1e6),method=method,...)
    opt$fW.par = opt$par
    opt$fW.par = c(1,opt$fW.par)  # account for redundancy
    opt$par = NULL
    opt$init.val = c(1,init.val)
    opt$shape=2
  } else if(!fixed.shape){
    opt <- optim(nll.pwlin.2d, par = init.val, r = r, 
                 r0w = r0w, w.adj.angles=w.adj.angles, locs=locs, norm=norm, marg=marg,
                 pen.norm=pen.norm, pen.adj=pen.adj,
                 fW.fit=F,joint.fit=F,pen.const=pen.const,fixed.shape=fixed.shape,
                 control=list(maxit=1e6),method=method,...)
    #method="L-BFGS-B",gr=NULL,lower=rep(0.01,length(locs)),upper=rep(10,length(locs))
    opt$fW.par = NULL  # didn't model angles
    opt$init.val = init.val
    opt$shape = opt$par[1]
    opt$par = opt$par[-1]
  }
  opt$aic = 2*(opt$value + length(opt$par))
  return(list(mle = opt$par,fW.mle = opt$fW.par, shape=opt$shape, nll = opt$value, convergence = opt$conv, 
              aic = opt$aic, init.val = opt$init.val))
}

bound.fit.2d = function(r,w,r0w,locs,norm=NULL,marg="pos",pen.norm="2", init.val=NULL, pen.adj=F, fixed.shape=TRUE,fW.fit=FALSE,joint.fit=FALSE,pen.const=0,method="BFGS",...){
  
  if(is.null(init.val)){
    init.val = rep(1,length(locs))
  }
  
  gfun=function(x,par) {gfun.2d(x,par,ref.angles=locs,norm=norm,marg=marg)}
  
  if(fW.fit & !joint.fit){
    stop("No need to bound anglar parameters if not jointly fitting.")
  }
  
  lik.custom = function(psi, r, r0w, w.adj.angles,
                        locs, norm, marg,
                        pen.norm, pen.adj,
                        fixed.pars, fixed.pars.idx,
                        fW.fit,joint.fit,pen.const){
    
    par.full = numeric(length(locs))
    par.full[fixed.pars.idx] = fixed.pars
    par.full[-fixed.pars.idx] = psi
    
    return(nll.pwlin.2d(psi=par.full,r=r,r0w=r0w,w.adj.angles=w.adj.angles, 
                        locs=locs,norm=norm,marg=marg,
                        pen.norm=pen.norm, pen.adj=pen.adj,
                        fW.fit=fW.fit,joint.fit=joint.fit,pen.const=pen.const))
  }
  
  nnodes=length(locs)
  w.adj.angles = which.adj.angles.2d(angles=w, locs, norm, marg)
  model.fit = fit.pwlin.2d(r=r, r0w=r0w,w=w,locs=locs,init.val=init.val,norm=norm,marg=marg,fW.fit=fW.fit,joint.fit=joint.fit,pen.const=pen.const)
  mle=model.fit$mle
  
  # stop("generalise this to use the 1 or 2 norm")

  fixed.pars.idx = c()
  
  if(marg=="pos"){
    keep.fitting = any(round(as.numeric(apply(cbind(locs,1-locs)/sapply(locs,gfun,par=mle),2,max)),6)!=1)
  } else if(marg=="Rd"){
    locs.cart = pol2cart.L1Rd(w=locs)
    keep.fitting = any(round(as.numeric(apply(locs.cart/sapply(locs,gfun,par=mle),2,min)),6)!=-1) &
      any(round(as.numeric(apply(locs.cart/sapply(locs,gfun,par=mle),2,max)),6)!=1)
  }
  
  max.iters=100
  iters=1
  while(keep.fitting){
  # is.bounded=F  # kind of like a dummy variable, will terminate the loop inside
  # while(!is.bounded){
    # simple re-scale satisfy marginal constraints
    
    # unitg = cbind(locs,1-locs)/gfun.2d(x=cbind(locs,1-locs),par=mle,ref.angles=locs)#apply(locs,1,gfun.3d,par=mle,ref.angles=locs)  # need to round here
    # 
    # # what is max along nodes
    # max.vals = apply(unitg,2, max)
    # which.max.vals = apply(unitg,2, which.max)  # row values
    # 
    # # print(fixed.pars.idx)
    # # print(max.vals)
    # 
    # max.df = data.frame(coord=c(1,2),max=max.vals,loc=which.max.vals)
    # max.lst = lapply(1:nrow(cbind(locs,1-locs)),function(loc){
    #   cond = max.df$loc==loc
    #   if(all(cond==F)){
    #     return(NULL)
    #   } else {
    #     return(list(loc=loc,
    #                 val=max(max.df$max[cond])))
    #   }
    # })
    # max.lst = max.lst[lapply(max.lst,length)>0]
    # 
    # if(all(round(max.vals,4)==1)){
    #   break  # algorithm complete, all comp. max. vals are 1
    # }
    # for(lst in max.lst){
    #   fixed.pars.idx = unique(sort(c(fixed.pars.idx,lst$loc)))
    #   mle[lst$loc] = mle[lst$loc]/lst$val
    # }
    # 
    # if(length(locs)==length(fixed.pars.idx)){
    #   break  # we fixed all the parameters
    # }
    
    # stop()
    if(marg=="pos"){
      if(0.5 %in% locs){
  
        idx.x  = locs >= 0.5
        idx.y  = locs <= 0.5
        idx.xy = locs == 0.5
        cm.x = max(locs/sapply(locs,gfun,par=mle))
        cm.y = max((1-locs)/sapply(locs,gfun,par=mle))
        cm.xy = 0.5/sapply(0.5,gfun,par=mle)
        if(cm.x==cm.xy & cm.y==cm.xy){
          which.cm = which(locs==0.5)
          mle[which.cm] = mle[which.cm]/cm.xy  # re-scale
          if(is.null(fixed.pars.idx)){
            fixed.pars.idx = which.cm
          } else {
            fixed.pars.idx = sort(c(fixed.pars.idx,which.cm))
          }
  
        } else {
          # max isn't on the diagonal
          # find the componentiwse max, and re-scale the corresponding MLEs
          idx.x = locs > 0.5
          idx.y = locs < 0.5
          unitg.x = locs/sapply(locs,gfun,par=mle)
          unitg.x[!idx.x] = 0
          unitg.y = (1-locs)/sapply(locs,gfun,par=mle)
          unitg.y[!idx.y] = 0
          which.cm.x = which.max(unitg.x)
          which.cm.y = which.max(unitg.y)
          mle[which.cm.x] = mle[which.cm.x]/(unitg.x[which.cm.x])
          mle[which.cm.y] = mle[which.cm.y]/(unitg.y[which.cm.y])
  
          # which mle's are re-scaled
          if(is.null(fixed.pars.idx)){
            fixed.pars.idx = sort(c(which.cm.x,which.cm.y))
          } else {
            fixed.pars.idx = sort(c(fixed.pars.idx,which.cm.x,which.cm.y))
          }
        }
      } else {
        # find the componentiwse max, and re-scale the corresponding MLEs
        idx.x = locs > 0.5
        idx.y = locs < 0.5
        unitg.x = locs/sapply(locs,gfun,par=mle)
        unitg.x[!idx.x] = 0
        unitg.y = (1-locs)/sapply(locs,gfun,par=mle)
        unitg.y[!idx.y] = 0
        which.cm.x = which.max(unitg.x)
        which.cm.y = which.max(unitg.y)
        mle[which.cm.x] = mle[which.cm.x]/(unitg.x[which.cm.x])
        mle[which.cm.y] = mle[which.cm.y]/(unitg.y[which.cm.y])
  
        # which mle's are re-scaled
        if(is.null(fixed.pars.idx)){
          fixed.pars.idx = sort(c(which.cm.x,which.cm.y))
        } else {
          fixed.pars.idx = unique(sort(c(fixed.pars.idx,which.cm.x,which.cm.y)))
        }
      }
    } else if(marg=="Rd"){
      unitg.coords = locs.cart/sapply(locs,gfun,par=mle)
      max.x.idx=which.max(unitg.coords[,1])
      min.x.idx=which.min(unitg.coords[,1])
      max.y.idx=which.max(unitg.coords[,2])
      min.y.idx=which.min(unitg.coords[,2])
      
      mle[max.x.idx] = abs(mle[max.x.idx]/(unitg.coords[max.x.idx,1]))
      mle[max.y.idx] = abs(mle[max.y.idx]/(unitg.coords[max.y.idx,2]))
      
      mle[min.x.idx] = abs(mle[min.x.idx]/(unitg.coords[min.x.idx,1]))
      mle[min.y.idx] = abs(mle[min.y.idx]/(unitg.coords[min.y.idx,2]))
      
      if(is.null(fixed.pars.idx)){
        fixed.pars.idx = sort(c(max.x.idx,max.y.idx,min.x.idx,min.y.idx))
      } else {
        fixed.pars.idx = unique(sort(c(fixed.pars.idx,max.x.idx,max.y.idx,min.x.idx,min.y.idx)))
      }
    }
    
    # re-fit the model
    if(fixed.shape & !fW.fit & !joint.fit){
      # only fit the conditional radial model
      init.vals=init.val[-fixed.pars.idx]  #rep(1,length(locs)-length(fixed.pars.idx))
      opt <- optim(lik.custom, par = init.vals, r = r, 
                   r0w = r0w, w.adj.angles=w.adj.angles, locs=locs, norm=norm, marg=marg,
                   pen.norm=pen.norm, pen.adj=pen.adj,
                   fW.fit=fW.fit,joint.fit=joint.fit,pen.const=pen.const,
                   fixed.pars=mle[fixed.pars.idx], fixed.pars.idx=fixed.pars.idx,
                   control=list(maxit=1e6),method=method)  #method="L-BFGS-B",gr=NULL,lower=rep(0.01,length(locs)-length(fixed.pars.idx)),upper=rep(10,length(locs)-length(fixed.pars.idx))
      opt$fW.par = NULL  # didn't model angles
    } else if(fixed.shape & fW.fit & joint.fit){
      # only fit the radial model and angular model together
      init.vals=init.val[-fixed.pars.idx]  #rep(1,length(locs)-length(fixed.pars.idx))
      opt <- optim(lik.custom, par = init.vals, r = r, 
                   r0w = r0w, w.adj.angles=w.adj.angles, locs=locs, norm=norm, marg=marg,
                   pen.norm=pen.norm, pen.adj=pen.adj,
                   fW.fit=fW.fit,joint.fit=joint.fit,pen.const=pen.const,
                   fixed.pars=mle[fixed.pars.idx], fixed.pars.idx=fixed.pars.idx,
                   control=list(maxit=1e6),method=method)
      opt$fW.par = opt$par
    } else {
      stop("ERROR in likelihood setup.")
    }
    
    mle[-fixed.pars.idx] = opt$par
    
    if(marg=="pos"){
      keep.fitting = any(round(as.numeric(apply(cbind(locs,1-locs)/sapply(locs,gfun,par=mle),2,max)),6)!=1)
    } else if(marg=="Rd"){
      locs.cart = pol2cart.L1Rd(w=locs)
      keep.fitting = any(round(as.numeric(apply(locs.cart/sapply(locs,gfun,par=mle),2,min)),6)!=-1) &
        any(round(as.numeric(apply(locs.cart/sapply(locs,gfun,par=mle),2,max)),6)!=1)
    }
    
    if(!keep.fitting){
      message(paste("comp. min:",paste(as.numeric(apply(locs.cart/sapply(locs,gfun,par=mle),2,min)),collapse=" ")))
      message(paste("comp. max:",paste(as.numeric(apply(locs.cart/sapply(locs,gfun,par=mle),2,max)),collapse=" ")))
      break
    }
    
    iters=iters+1
    if(iters>max.iters){
      message("max iters in bounding reached")
      break
    }
    
  }
  # return(list(mle=mle,
  #             idx=fixed.pars.idx,
  #             nll=opt$value))
  return(list(mle =mle,
       fixed.pars.idx=fixed.pars.idx,
       fW.mle = NA, 
       shape=2, 
       nll = opt$value, 
       convergence = opt$conv,
       init.val = init.val))
}


######################################################################

# 3-d model fitting for when w is on the simplex.

nll.pwlin.3d = function(psi, r, r0w, w.adj.angles, locs, pen.norm, pen.adj, fW.fit, joint.fit,  pos.par=TRUE, pen.const=0){
  
  # ensure we have no negative parameters
  if(pos.par) {
    if (any(psi <= 0)){
      return(1e+11)
    }
  }
  
  if(fW.fit & joint.fit){
    rate <- sapply(w.adj.angles, function(lst){
      angle = lst$w
      which.angles = lst$idx.locs
      if(any(is.na(angle)) | all(which.angles==0)){  # if angle is not in the positive orthant OR if angle lies on line
        return(NA)
      } else {
        return(gfun.simple.d3pwlin(xyz=angle,
                                   par=psi[which.angles],
                                   par.locs=locs[which.angles,]))
      }
    })  
    ll1 <- dgamma(r, shape = 3, rate = rate, log = T)
    ll2 <- log(pgamma(r0w, shape = 3, rate = rate, lower.tail = F))
    G.vol = G.vol.3d(psi,locs)
    llw = -log(3*G.vol) -3*log(rate)
    any.na = is.na(ll1) | is.na(ll2) | is.na(llw)
    NLL = -sum(ll1[!any.na]) + sum(ll2[!any.na]) - sum(llw[!any.na])
  } else if(!fW.fit & !joint.fit){
    rate <- sapply(w.adj.angles, function(lst){
      angle = lst$w
      which.angles = lst$idx.locs
      if(any(is.na(angle)) | all(which.angles==0)){  # if angle is not in the positive orthant OR if angle lies on line
        return(NA)
      } else {
        return(gfun.simple.d3pwlin(xyz=angle,
                                   par=psi[which.angles],
                                   par.locs=locs[which.angles,]))
      }
    })  
    ll1 <- dgamma(r, shape = 3, rate = rate, log = T)
    ll2 <- log(pgamma(r0w, shape = 3, rate = rate, lower.tail = F))
    any.na = is.na(ll1) | is.na(ll2)
    NLL = -sum(ll1[!any.na]) + sum(ll2[!any.na])
  } else if(fW.fit & !joint.fit){
    psi = c(1,psi)  # account for redundancy
    rate <- sapply(w.adj.angles, function(lst){
      angle = lst$w
      which.angles = lst$idx.locs
      if(any(is.na(angle)) | all(which.angles==0)){  # if angle is not in the positive orthant OR if angle lies on line
        return(NA)
      } else {
        return(gfun.simple.d3pwlin(xyz=angle,
                                   par=psi[which.angles],
                                   par.locs=locs[which.angles,]))
      }
    })  
    G.vol = G.vol.3d(psi,locs)
    llw = -log(3*G.vol) -3*log(rate)
    NLL = -sum(llw,na.rm=T)
  }
  if(pen.const>0){
    
    # locs = rbind(diag(3),rep(1/3,3))
    del.tri = geometry::delaunayn(p=locs[,-3], output.options=TRUE)
    tri = del.tri$tri
    
    locs.scaled = locs*psi
    
    grad.g = lapply(1:nrow(tri),function(row.idx){
      idx = tri[row.idx,]
      idx.fix = idx[1]
      idx.rest = idx[-1]
      coplanar.mat = do.call(rbind,lapply(idx.rest, function(ii){
        return(locs.scaled[idx.fix,] - locs.scaled[ii,])
      }))
      norm.vec = c(det(coplanar.mat[,-1]),-det(coplanar.mat[,-2]),det(coplanar.mat[,-3]))
      grad.g.val = norm.vec / sum(norm.vec * locs.scaled[idx.fix,])
      return(grad.g.val)
    })
    # pen = 0
    idx.vecs = NULL
    for(i in 1:nrow(locs)){
      which.has.i = apply(tri,1,function(ii){
        i %in% ii
      })
      if(sum(which.has.i)==1){
        next  # only 1 face, can't compare gradient to another
      } else if(sum(which.has.i)>1){
        which.grad.gs = (1:length(grad.g))[which.has.i]#do.call(rbind,(1:length(grad.g))[which.has.i])
        idx.combs = combn(1:length(which.grad.gs),2)
        idx.vec = do.call(rbind,lapply(1:ncol(idx.combs),function(i.col){
          return(sort(which.grad.gs[idx.combs[,i.col]]))
          # idx.vec = idx.combs[,i.col]
          # idx.vecs[[count]] = idx.vec
          # count=count+1
          # diff(grad.gs[idx.vec,])
        }))
        idx.vecs = rbind(idx.vecs,idx.vec)
        # pen.val = apply(pen.val^2,1,sum)
        # pen = pen + sum(pen.val)
      }
    }
    idx.vecs = idx.vecs[!duplicated(idx.vecs), ]
    grad.g.mat = do.call(rbind,grad.g)
    pen = apply(idx.vecs,1,function(ii){
      pp = diff(grad.g.mat[ii,])
      if(pen.norm=="2"){
        return(sum(pp^2))
      } else if(pen.norm=="1"){
        return(sum(abs(pp)))
      }
    })
    if(pen.adj){
      # stop("to do.")
      
      # a=do.call(rbind,lapply(w.adj.angles,function(lst) sort(lst$idx.locs)))
      # b=t(apply(tri,1,function(vec) sort(vec)))
      
      num = apply(tri,1,function(which.locs){
          sum(sapply(w.adj.angles,function(lst) all(sort(lst$idx.locs) == sort(which.locs))))
      })
      
      # n.DT.region.3d(w.adj.angles,locs[-3])

      # do.call(rbind,lapply(w.adj.angles,function(lst) lst$idx.locs))
      
      pen.multiplier = apply(idx.vecs,1,function(ii) sum(num[ii]))
      pen.multiplier = ifelse(pen.multiplier==0,1,pen.multiplier)
      
      pen = pen/pen.multiplier
    }
    pen = sum(pen)
    return(NLL+(pen.const*pen))
  } else {
    return(NLL)
  }
}

fit.pwlin.3d = function(r, r0w, w, locs, init.val=NULL, pen.const=0, 
                        pen.norm="2", pen.adj=F, fixed.shape=TRUE, 
                        fW.fit=FALSE, joint.fit=FALSE,bound=FALSE,
                        method="BFGS",maxit=1e6,reltol=1e-5,...){
  if(is.null(init.val)){
    init.val = rep(1,nrow(locs))
  }
  w.adj.angles = which.adj.angles.3d(w,locs)
  if(fixed.shape & !fW.fit & !joint.fit){
    # fit the conditional radial model only
    opt <- optim(nll.pwlin.3d, par = init.val, r = r, 
                 r0w = r0w, w.adj.angles=w.adj.angles, locs=locs,
                 fW.fit=F,joint.fit=F,pen.const=pen.const,
                 pen.norm=pen.norm, pen.adj=pen.adj, 
                 control=list(maxit=maxit,reltol=reltol),method=method,...)
    opt$fW.par = NULL  # didn't model angles
  } else if(fixed.shape & fW.fit & joint.fit){
    # fit the radial model and angular model together
    opt <- optim(nll.pwlin.3d, par = init.val, r = r, 
                 r0w = r0w, w.adj.angles=w.adj.angles, locs=locs,
                 fW.fit=T,joint.fit=T,pen.const=pen.const,
                 pen.norm=pen.norm, pen.adj=pen.adj, 
                 control=list(maxit=maxit,reltol=reltol),method=method,...)
    opt$fW.par = opt$par
  } else if(fixed.shape & fW.fit & !joint.fit){
    # fit the angular model only
    init.val = init.val[-1]  # remove redundancy, fit the first parameter to 1.
    opt <- optim(nll.pwlin.3d, par = init.val, r = r, 
                 r0w = r0w, w.adj.angles=w.adj.angles, locs=locs,
                 fW.fit=T,joint.fit=F,pen.const=pen.const,
                 pen.norm=pen.norm, pen.adj=pen.adj, 
                 control=list(maxit=maxit,reltol=reltol),method=method,...)
    opt$fW.par = opt$par
    opt$fW.par = c(1,opt$fW.par)  # account for redundancy
    opt$par = NULL
    init.val = c(1,init.val)
  } else if(!fixed.shape){
    stop("fitting shape not implemented.")
  }
  opt$init.val = init.val#c(3,init.val)
  opt$aic = 2*(opt$value + length(opt$par))
  return(list(mle = opt$par, fW.mle = opt$fW.par, shape=3, 
              nll = opt$value, convergence = opt$conv, 
              aic = opt$aic, init.val = opt$init.val))
}


bound.fit.3d = function(r,w,r0w,locs,init.val=NULL,pen.const=0,pen.norm="2",
                        pen.adj=F,fixed.shape=TRUE,fW.fit=F,joint.fit=F,
                        method="BFGS",...){
  
  if(fW.fit & !joint.fit){
    stop("No need to bound anglar parameters if not jointly fitting.")
  }
  
  if(is.null(init.val)){
    init.val = rep(1,nrow(locs))
  }
    
  # locs.df = data.frame(locs[,-3],row.names=NULL)
  # names(locs.df) = c("x","y")
  w.adj.angles = which.adj.angles.3d(angles=w,locs=locs)
  
  # fixed.pars.idx=NULL
  opt <- fit.pwlin.3d(r=r,w=w,r0w=r0w,locs=locs,init.val=init.val,
                      pen.norm=pen.norm, pen.adj=pen.adj, 
                      fW.fit=fW.fit,joint.fit=joint.fit,pen.const=pen.const,...)
  mle = opt$mle
  
  lik.custom = function(psi, r, r0w, w.adj.angles, locs = locs, fixed.pars, 
                        fixed.pars.idx, fW.fit, joint.fit, pos.par = TRUE,
                        pen.const,pen.norm,pen.adj){
    
    psi.full = numeric(nrow(locs))
    psi.full[fixed.pars.idx] = fixed.pars
    psi.full[-fixed.pars.idx] = psi
    
    # if(pos.par) {
    #   if (any(psi.full <= 0)){
    #     return(1e+11)
    #   }
    # }
    
    return(nll.pwlin.3d(psi=psi.full,r=r,r0w=r0w,w.adj.angles=w.adj.angles, 
                        locs=locs, fW.fit=fW.fit, joint.fit=joint.fit, pos.par=TRUE,
                        pen.const=pen.const,pen.norm=pen.norm,pen.adj=pen.adj))
    
  }
  fixed.pars.idx = c()
  is.bounded=F  # kind of like a dummy variable, will terminate the loop inside
  i=1
  max.iter=nrow(locs)
  while(!is.bounded){
    # print(i)
    
    # evaluate at nodes
    unitg = locs/gfun.3d(x=locs,par=mle,ref.angles=locs)#apply(locs,1,gfun.3d,par=mle,ref.angles=locs)  # need to round here
    
    # what is max along nodes
    max.vals = apply(unitg,2, max)
    which.max.vals = apply(unitg,2, which.max)  # row values
    
    max.df = data.frame(coord=c(1,2,3),max=max.vals,loc=which.max.vals)
    max.lst = lapply(1:nrow(locs),function(loc){
      cond = max.df$loc==loc
      vals=max.df$max[cond]
      if(all(cond==F)){
        return(NULL)
      } else {
        return(list(loc=loc,
                    val=max(vals)))
      }
    })
    max.lst = max.lst[lapply(max.lst,length)>0]
        
    if(all(round(max.vals,4)==1)){
      break  # algorithm complete, all comp. max. vals are 1
    }
    for(lst in max.lst){
      fixed.pars.idx = unique(sort(c(fixed.pars.idx,lst$loc)))
      mle[lst$loc] = mle[lst$loc]/lst$val
    }
    
    if(nrow(locs)==length(fixed.pars.idx)){
      break  # we fixed all the parameters
    }
    
    if(fixed.shape & !fW.fit & !joint.fit){
      # fit the conditional radial model only
      init.vals = init.val[-fixed.pars.idx] #rep(1,nrow(locs)-length(fixed.pars.idx))
      opt <- optim(lik.custom, par = init.vals, r = r, 
                   r0w = r0w, w.adj.angles=w.adj.angles, locs=locs,
                   fW.fit=F,joint.fit=F,pen.const=pen.const,
                   pen.norm=pen.norm, pen.adj=pen.adj,
                   fixed.pars=mle[fixed.pars.idx], fixed.pars.idx=fixed.pars.idx, 
                   control=list(maxit=1e6,reltol=1e-5),method=method,...)
      opt$fW.par = NULL  # didn't model angles
    } else if(fixed.shape & fW.fit & joint.fit){
      # fit the radial model and angular model together
      init.vals = init.val[-fixed.pars.idx] #rep(1,nrow(locs)-length(fixed.pars.idx))
      opt <- optim(lik.custom, par = init.vals, r = r, 
                   r0w = r0w, w.adj.angles=w.adj.angles, locs=locs,
                   fW.fit=T,joint.fit=T,pen.const=pen.const,
                   pen.norm=pen.norm, pen.adj=pen.adj,
                   fixed.pars=mle[fixed.pars.idx], fixed.pars.idx=fixed.pars.idx, 
                   control=list(maxit=1e6,reltol=1e-5),method=method,...)
      opt$fW.par = opt$par
    } else {
      stop("ERROR in likelihood setup")
    }
    mle[-fixed.pars.idx] = opt$par
    i=i+1
    if(i>max.iter){
      stop("Bounding algorithm has reached the maximum number of iterations.")
    }
  }
  return(list(mle = mle, fW.mle = NA, shape=3, 
              fixed.pars.idx=fixed.pars.idx,
              nll = opt$value, convergence = opt$conv, 
              init.val = init.val))
}


######################################################################

# 4-d model fitting for when w is on the simplex.

nll.pwlin.4d = function(psi, r, r0w, w.adj.angles, locs, fW.fit, joint.fit, pen.const=0, pos.par=TRUE,
                        del.tri, pen.adj=FALSE, num.DT=NULL){
  
  # ensure we have no negative parameters
  if(pos.par) {
    if (any(psi <= 0)){
      return(1e+11)
    }
  }
  
  if(fW.fit & joint.fit){
    rate <- pwlin.g.vals.4d(w.adj.angles=w.adj.angles,par=psi,par.locs=locs)
    ll1 <- dgamma(r, shape = 4, rate = rate, log = T)
    ll2 <- log(pgamma(r0w, shape = 4, rate = rate, lower.tail = F))
    G.vol = G.vol.4d(psi,locs)
    llw = -log(4*G.vol) -4*log(rate)
    NLL = -sum(ll1) + sum(ll2) - sum(llw)
  } else if(!fW.fit & !joint.fit){
    rate <- pwlin.g.vals.4d(w.adj.angles=w.adj.angles,par=psi,par.locs=locs)
    ll1 <- dgamma(r, shape = 4, rate = rate, log = T)
    ll2 <- log(pgamma(r0w, shape = 4, rate = rate, lower.tail = F))
    NLL = -sum(ll1) + sum(ll2)
  } else if(fW.fit & !joint.fit){
    psi = c(1,psi)  # account for redundancy
    rate <- pwlin.g.vals.4d(w.adj.angles=w.adj.angles,par=psi,par.locs=locs)
    G.vol = G.vol.4d(psi,locs)
    llw = -log(4*G.vol) -4*log(rate)
    NLL = -sum(llw)
  }
  if(pen.const>0){
    # stop("make sure the penalty fitting works in 4d")
    # locs = rbind(diag(3),rep(1/3,3))
    tri = del.tri$tri
    
    locs.scaled = locs*psi
    
    if(pen.adj){
      # num.DT = n.DT.region(w.adj.angles,locs)
      nums = sapply(1:nrow(tri),function(row.idx){
        idx = tri[row.idx,]
        which.reg = sapply(num.DT,function(lst) all(lst$loc == sort(idx)))
        if(sum(which.reg)==0){
          return(1)
        } else {
          return(num.DT[[which(which.reg)]]$freq)
        }
      })
    }
    
    grad.g = lapply(1:nrow(tri),function(row.idx){
      idx = tri[row.idx,]
      idx.fix = idx[1]
      idx.rest = idx[-1]
      coplanar.mat = do.call(rbind,lapply(idx.rest, function(ii){
        return(locs.scaled[idx.fix,] - locs.scaled[ii,])
      }))
      norm.vec = c(det(coplanar.mat[,-1]),-det(coplanar.mat[,-2]),det(coplanar.mat[,-3]),-det(coplanar.mat[,-4]))
      
      grad.g.val = norm.vec / sum(norm.vec * locs.scaled[idx.fix,])
      
      return(grad.g.val)
    })
    # pen = 0
    idx.vecs = NULL
    for(i in 1:nrow(locs)){
      which.has.i = apply(tri,1,function(ii){
        i %in% ii
      })
      if(sum(which.has.i)==1){
        next  # only 1 face, can't compare gradient to another
      } else if(sum(which.has.i)>1){
        which.grad.gs = (1:length(grad.g))[which.has.i]
        idx.combs = combn(1:length(which.grad.gs),2)
        idx.vec = do.call(rbind,lapply(1:ncol(idx.combs),function(i.col){
          return(sort(which.grad.gs[idx.combs[,i.col]]))
        }))
        idx.vecs = rbind(idx.vecs,idx.vec)
      }
    }
    idx.vecs = idx.vecs[!duplicated(idx.vecs), ]
    grad.g.mat = do.call(rbind,grad.g)
    pen = apply(idx.vecs,1,function(ii){
      pp = diff(grad.g.mat[ii,])
      if(pen.adj){
        adj.const = sum(nums[ii])
        return(sum(pp^2)/adj.const)
      } else {
        return(sum(pp^2))
      }
    })
    pen = sum(pen)
    return(NLL+(pen.const*pen))
  } else {
    return(NLL)
  }
}

fit.pwlin.4d = function(r, r0w, w, locs, 
                        init.val=NULL, fixed.shape=TRUE, fW.fit=FALSE, joint.fit=FALSE,bound=FALSE,method="BFGS",
                        pen.const=0,pen.adj=FALSE,
                        ...){
  if(is.null(init.val)){
    init.val = rep(1,nrow(locs))
  }
  
  w.adj.angles = which.adj.angles.4d(w,locs)
  del.tri = geometry::delaunayn(p=locs[,-4], output.options=TRUE)
  num.DT = n.DT.region(w.adj.angles,locs)
  
  if(fixed.shape & !fW.fit & !joint.fit){
    # fit the conditional radial model only
    opt <- optim(nll.pwlin.4d, par = init.val, r = r, 
                 r0w = r0w, w.adj.angles=w.adj.angles, locs=locs,pen.adj=pen.adj,
                 fW.fit=F,joint.fit=F,pen.const=pen.const,
                 del.tri=del.tri,num.D=num.DT,
                 control=list(maxit=1e6,reltol=1e-5),method=method,...)
    opt$fW.par = NULL  # didn't model angles
  } else if(fixed.shape & fW.fit & joint.fit){
    # fit the radial model and angular model together
    opt <- optim(nll.pwlin.4d, par = init.val, r = r, 
                 r0w = r0w, w.adj.angles=w.adj.angles, locs=locs,pen.adj=pen.adj,
                 fW.fit=T,joint.fit=T,pen.const=pen.const,
                 del.tri=del.tri,num.D=num.DT,
                 control=list(maxit=1e6,reltol=1e-5),method=method,...)
    opt$fW.par = opt$par
  } else if(fixed.shape & fW.fit & !joint.fit){
    # fit the angular model only
    init.val = init.val[-1]  # remove redundancy, fit the first parameter to 1.
    opt <- optim(nll.pwlin.4d, par = init.val, r = r, 
                 r0w = r0w, w.adj.angles=w.adj.angles, locs=locs,pen.adj=pen.adj,
                 fW.fit=T,joint.fit=F,pen.cons=pen.const,
                 del.tri=del.tri,num.D=num.DT,
                 control=list(maxit=1e6,reltol=1e-5),method=method,...)
    opt$fW.par = opt$par
    opt$fW.par = c(1,opt$fW.par)  # account for redundancy
    opt$par = NULL
  } else if(!fixed.shape){
    stop("fitting shape not implemented.")
  }
  opt$init.val = init.val#c(3,init.val)
  opt$aic = 2*(opt$value + length(opt$par))
  return(list(mle = opt$par, fW.mle = opt$fW.par, shape=4, nllh = opt$value, convergence = opt$conv, 
              aic = opt$aic, init.val = opt$init.val))
}


bound.fit.4d = function(r,w,r0w,locs,init.val=NULL, fixed.shape=TRUE,fW.fit=F,joint.fit=F,method="BFGS",pen.const=0,pen.adj=FALSE,...){
  
  if(fW.fit & !joint.fit){
    stop("No need to bound anglar parameters if not jointly fitting.")
  }
  
  if(is.null(init.val)){
    init.val = rep(1,nrow(locs))
  }
  
  # fixed.pars.idx=NULL
  opt <- fit.pwlin.4d(r=r,w=w,r0w=r0w,locs=locs,init.val=init.val,fW.fit=fW.fit,joint.fit=joint.fit,pen.const=pen.const,pen.adj=pen.adj,...)
  mle = opt$mle
  
  w.adj.angles = which.adj.angles.4d(angles=w,locs=locs)
  del.tri = geometry::delaunayn(p=locs[,-4], output.options=TRUE)
  num.DT = n.DT.region(w.adj.angles,locs)
  
  lik.custom = function(psi, r, r0w, w.adj.angles, num.DT, del.tri, locs = locs, fixed.pars, 
                        fixed.pars.idx, fW.fit, joint.fit, pos.par = TRUE, pen.adj=FALSE,
                        pen.const=0){
    
    psi.full = numeric(nrow(locs))
    psi.full[fixed.pars.idx] = fixed.pars
    psi.full[-fixed.pars.idx] = psi
    
    # if(pos.par) {
    #   if (any(psi.full <= 0)){
    #     return(1e+11)
    #   }
    # }
    
    return(nll.pwlin.4d(psi=psi.full,r=r,r0w=r0w,w.adj.angles=w.adj.angles, num.DT=num.DT, del.tri=del.tri,
                        locs=locs, fW.fit=fW.fit, joint.fit=joint.fit, pos.par=TRUE,pen.adj=pen.adj,
                        pen.const=pen.const))
    
  }
  fixed.pars.idx = c()
  is.bounded=F   # kind of like a dummy variable, will terminate the loop inside
  while(!is.bounded){
    # print(i)
    
    # evaluate at nodes
    unitg = locs/gfun.4d(x=locs,par=mle,ref.angles=locs)#apply(locs,1,gfun.4d,par=mle,ref.angles=locs)  # need to round here
    
    # what is max along nodes
    max.vals = apply(unitg,2, max)
    which.max.vals = apply(unitg,2, which.max)  # row values
    
    max.df = data.frame(coord=c(1,2,3,4),max=max.vals,loc=which.max.vals)
    max.lst = lapply(1:nrow(locs),function(loc){
      cond = max.df$loc==loc
      if(all(cond==F)){
        return(NULL)
      } else {
        return(list(loc=loc,
                    val=max(max.df$max[cond])))
      }
    })
    max.lst = max.lst[lapply(max.lst,length)>0]
    
    if(all(round(max.vals,4)==1)){
      break  # algorithm complete, all comp. max. vals are 1
    }
    for(lst in max.lst){
      fixed.pars.idx = unique(sort(c(fixed.pars.idx,lst$loc)))
      mle[lst$loc] = mle[lst$loc]/lst$val
    }
    
    if(nrow(locs)==length(fixed.pars.idx)){
      break  # we fixed all the parameters
    }
    
    if(fixed.shape & !fW.fit & !joint.fit){
      # fit the conditional radial model only
      init.vals = init.val[-fixed.pars.idx]
      opt <- optim(lik.custom, par = init.vals, r = r, 
                   r0w = r0w, w.adj.angles=w.adj.angles, locs=locs,num.DT=num.DT,
                   fW.fit=F,joint.fit=F,pen.const=pen.const,del.tri=del.tri,pen.adj=pen.adj,
                   fixed.pars=mle[fixed.pars.idx], fixed.pars.idx=fixed.pars.idx, 
                   control=list(maxit=1e6,reltol=1e-5),method=method,...)
      opt$fW.par = NULL  # didn't model angles
    } else if(fixed.shape & fW.fit & joint.fit){
      # fit the radial model and angular model together
      init.vals = init.val[-fixed.pars.idx]
      opt <- optim(lik.custom, par = init.vals, r = r, 
                   r0w = r0w, w.adj.angles=w.adj.angles, locs=locs,num.DT=num.DT,
                   fW.fit=T,joint.fit=T,pen.const=pen.const,del.tri=del.tri,pen.adj=pen.adj,
                   fixed.pars=mle[fixed.pars.idx], fixed.pars.idx=fixed.pars.idx, 
                   control=list(maxit=1e6,reltol=1e-5),method=method,...)
      opt$fW.par = opt$par
    } else {
      stop("ERROR in likelihood setup")
    }
    mle[-fixed.pars.idx] = opt$par
    # i=i+1
  }
  return(list(mle=mle,
              fixed.pars.idx=fixed.pars.idx))
}

######################################################################

# general d-dimensional model fitting for when w is on the simplex.

pwl.L1.pen = function(psi,locs){
  require(geometry)
  pen.vals = sapply(1:length(psi), function(i){
    # print(locs[i,])
    psi.i = psi[i]
    locs.i = locs[i,]
    psi.rest = psi[-i]
    locs.rest = locs[-i,]
    suppressWarnings({
      g.val.i = gfun.pwl(x=locs.i,par = psi.rest,ref.angles=locs.rest)
    })
    return(abs(psi.i - (1/g.val.i)))
  })
  return(sum(pen.vals,na.rm=T))
}
pwl.L2.pen = function(psi,locs,del.tri){
  require(geometry)
  tri = del.tri$tri
  
  locs.scaled = locs*psi
  
  num.cols = dim(locs)[2]
  
  grad.g = lapply(1:nrow(tri),function(row.idx){
    idx = tri[row.idx,]
    idx.fix = idx[1]
    idx.rest = idx[-1]
    coplanar.mat = do.call(rbind,lapply(idx.rest, function(ii){
      return(locs.scaled[idx.fix,] - locs.scaled[ii,])
    }))
    # norm.vec = c(det(coplanar.mat[,-1]),-det(coplanar.mat[,-2]),det(coplanar.mat[,-3]),-det(coplanar.mat[,-4]))
    norm.vec = suppressWarnings({
      c(1,-1)*sapply(c(1:num.cols),function(i){det(coplanar.mat[,-i])})
    })
    grad.g.val = norm.vec / sum(norm.vec * locs.scaled[idx.fix,])
    return(grad.g.val)
  })
  # pen = 0
  idx.vecs = NULL
  for(i in 1:nrow(locs)){
    which.has.i = apply(tri,1,function(ii){
      i %in% ii
    })
    if(sum(which.has.i)==1){
      next  # only 1 face, can't compare gradient to another
    } else if(sum(which.has.i)>1){
      which.grad.gs = (1:length(grad.g))[which.has.i]
      idx.combs = combn(1:length(which.grad.gs),2)
      idx.vec = do.call(rbind,lapply(1:ncol(idx.combs),function(i.col){
        return(sort(which.grad.gs[idx.combs[,i.col]]))
      }))
      idx.vecs = rbind(idx.vecs,idx.vec)
    }
  }
  idx.vecs = idx.vecs[!duplicated(idx.vecs), ]
  grad.g.mat = do.call(rbind,grad.g)
  pen = apply(idx.vecs,1,function(ii){
    pp = diff(grad.g.mat[ii,])
    return(sum(pp^2))
  })
  return(sum(pen))
}

nll.pwlin = function(psi, r, r0w, w.adj.angles, del.tri, locs, fW.fit, joint.fit, pen.const.L1=0, pen.const.L2=0, pos.par=TRUE){
  
  # ensure we have no negative parameters
  if(pos.par) {
    if (any(psi <= 0)){
      return(1e+11)
    }
  }
  
  # # REMOVE THIS...
  # if (any(psi > 5)){
  #   return(1e+11)
  # }
  
  num.cols = length(w.adj.angles[[1]]$w)
  
  if(fW.fit & joint.fit){
    rate <- pwlin.g.vals(w.adj.angles=w.adj.angles,par=psi,par.locs=locs)
    ll1 <- dgamma(r, shape = num.cols, rate = rate, log = T)
    ll2 <- log(pgamma(r0w, shape = num.cols, rate = rate, lower.tail = F))
    G.vol = G.vol(psi,locs)
    llw = -log(num.cols*G.vol) -num.cols*log(rate)
    NLL = -sum(ll1) + sum(ll2) - sum(llw)
  } else if(!fW.fit & !joint.fit){
    rate <- pwlin.g.vals(w.adj.angles=w.adj.angles,par=psi,par.locs=locs)
    ll1 <- dgamma(r, shape = num.cols, rate = rate, log = T)
    ll2 <- log(pgamma(r0w, shape = num.cols, rate = rate, lower.tail = F))
    NLL = -sum(ll1) + sum(ll2)
  } else if(fW.fit & !joint.fit){
    psi = c(1,psi)  # account for redundancy
    rate <- pwlin.g.vals(w.adj.angles=w.adj.angles,par=psi,par.locs=locs)
    G.vol = G.vol(psi,locs)
    llw = -log(num.cols*G.vol) -num.cols*log(rate)
    NLL = -sum(llw)
  }
  if(pen.const.L1>0){
    L1.pen.val = pwl.L1.pen(psi=psi,locs=locs)
    NLL = NLL+(pen.const.L1*L1.pen.val)
  }
  if(pen.const.L2>0){
    L2.pen.val = pwl.L2.pen(psi=psi,locs=locs,del.tri=del.tri)
    NLL = NLL+(pen.const.L2*L2.pen.val)
  }
  # print(psi)
  # print(NLL)
  return(NLL)
}

fit.pwlin = function(r, r0w, w, locs, 
                        init.val=NULL, fixed.shape=TRUE, fW.fit=FALSE, joint.fit=FALSE,bound=FALSE,method="BFGS",
                        pen.const.L1=0,pen.const.L2=0,...){
  if(is.null(init.val)){
    init.val = rep(1,nrow(locs))
  }
  
  w.adj.angles = which.adj.angles(w,locs)
  
  num.cols = length(w.adj.angles[[1]]$w)
  del.tri = geometry::delaunayn(p=locs[,-num.cols], output.options=TRUE)
  
  if(fixed.shape & !fW.fit & !joint.fit){
    # fit the conditional radial model only
    opt <- optim(nll.pwlin, par = init.val, r = r, 
                 r0w = r0w, w.adj.angles=w.adj.angles, locs=locs,
                 fW.fit=F,joint.fit=F,
                 pen.const.L1=pen.const.L1,pen.const.L2=pen.const.L2,
                 del.tri=del.tri,
                 control=list(maxit=1e6,reltol=1e-5),method=method,...)
    opt$fW.par = NULL  # didn't model angles
  } else if(fixed.shape & fW.fit & joint.fit){
    # fit the radial model and angular model together
    opt <- optim(nll.pwlin, par = init.val, r = r, 
                 r0w = r0w, w.adj.angles=w.adj.angles, locs=locs,
                 fW.fit=T,joint.fit=T,
                 pen.const.L1=pen.const.L1,pen.const.L2=pen.const.L2,
                 del.tri=del.tri,
                 control=list(maxit=1e6,reltol=1e-5),method=method,...)
    opt$fW.par = opt$par
  } else if(fixed.shape & fW.fit & !joint.fit){
    # fit the angular model only
    init.val = init.val[-1]  # remove redundancy, fit the first parameter to 1.
    opt <- optim(nll.pwlin, par = init.val, r = r, 
                 r0w = r0w, w.adj.angles=w.adj.angles, locs=locs,
                 fW.fit=T,joint.fit=F,
                 pen.const.L1=pen.const.L1,pen.const.L2=pen.const.L2,
                 del.tri=del.tri,
                 control=list(maxit=1e6,reltol=1e-5),method=method,...)
    opt$fW.par = opt$par
    opt$fW.par = c(1,opt$fW.par)  # account for redundancy
    opt$par = NULL
  } else if(!fixed.shape){
    stop("fitting shape not implemented.")
  }
  opt$init.val = init.val#c(3,init.val)
  opt$aic = 2*(opt$value + length(opt$par))
  return(list(mle = opt$par, fW.mle = opt$fW.par, shape=dim(w)[2], 
              nllh = opt$value, convergence = opt$conv, 
              aic = opt$aic, init.val = opt$init.val))
}



bound.fit = function(r,w,r0w,locs,init.val=NULL,fixed.shape=TRUE,fW.fit=F,joint.fit=F,method="BFGS",pen.const=0,...){
  
  if(fW.fit & !joint.fit){
    stop("No need to bound anglar parameters if not jointly fitting.")
  }
  
  if(is.null(init.val)){
    init.val = rep(1,nrow(locs))
  }
  
  num.cols = dim(w)[2]
  
  w.adj.angles = which.adj.angles(angles=w,locs=locs)
  del.tri = geometry::delaunayn(p=locs[,-num.cols], output.options=TRUE)
  
  # fixed.pars.idx=NULL
  opt <- fit.pwlin(r=r,w=w,r0w=r0w,locs=locs,init.val=init.val,fW.fit=fW.fit,joint.fit=joint.fit,pen.const=pen.const,...)
  mle = opt$mle
  
  lik.custom = function(psi, r, r0w, w.adj.angles, locs = locs, fixed.pars, 
                        fixed.pars.idx, fW.fit, joint.fit, pos.par = TRUE,
                        pen.const=0,del.tri=del.tri){
    
    psi.full = numeric(nrow(locs))
    psi.full[fixed.pars.idx] = fixed.pars
    psi.full[-fixed.pars.idx] = psi
    
    # if(pos.par) {
    #   if (any(psi.full <= 0)){
    #     return(1e+11)
    #   }
    # }
    
    return(nll.pwlin(psi=psi.full,r=r,r0w=r0w,w.adj.angles=w.adj.angles, 
                     locs=locs, fW.fit=fW.fit, joint.fit=joint.fit, pos.par=TRUE,
                     pen.const=pen.const,del.tri=del.tri))
    
  }
  fixed.pars.idx = c()
  is.bounded=F   # kind of like a dummy variable, will terminate the loop inside
  while(!is.bounded){
    # print(i)
    
    # evaluate at nodes
    unitg = locs/gfun.pwl(x=locs,par=mle,ref.angles=locs)#apply(locs,1,gfun.4d,par=mle,ref.angles=locs)  # need to round here
    
    # what is max along nodes
    max.vals = apply(unitg,2, max)
    which.max.vals = apply(unitg,2, which.max)  # row values
    
    max.df = data.frame(coord=c(1:num.cols),max=max.vals,loc=which.max.vals)
    max.lst = lapply(1:nrow(locs),function(loc){
      cond = max.df$loc==loc
      if(all(cond==F)){
        return(NULL)
      } else {
        return(list(loc=loc,
                    val=max(max.df$max[cond])))
      }
    })
    max.lst = max.lst[lapply(max.lst,length)>0]
    
    if(all(round(max.vals,num.cols)==1)){
      break  # algorithm complete, all comp. max. vals are 1
    }
    for(lst in max.lst){
      fixed.pars.idx = unique(sort(c(fixed.pars.idx,lst$loc)))
      mle[lst$loc] = mle[lst$loc]/lst$val
    }
    
    if(nrow(locs)==length(fixed.pars.idx)){
      break  # we fixed all the parameters
    }
    
    if(fixed.shape & !fW.fit & !joint.fit){
      # fit the conditional radial model only
      init.vals = init.val[-fixed.pars.idx]
      opt <- optim(lik.custom, par = init.vals, r = r, 
                   r0w = r0w, w.adj.angles=w.adj.angles, locs=locs,
                   fW.fit=F,joint.fit=F,pen.const=pen.const,del.tri=del.tri,
                   fixed.pars=mle[fixed.pars.idx], fixed.pars.idx=fixed.pars.idx, 
                   control=list(maxit=1e6,reltol=1e-5),method=method,...)
      opt$fW.par = NULL  # didn't model angles
    } else if(fixed.shape & fW.fit & joint.fit){
      # fit the radial model and angular model together
      init.vals = init.val[-fixed.pars.idx]
      opt <- optim(lik.custom, par = init.vals, r = r, 
                   r0w = r0w, w.adj.angles=w.adj.angles, locs=locs,
                   fW.fit=T,joint.fit=T,pen.const=pen.const,del.tri=del.tri,
                   fixed.pars=mle[fixed.pars.idx], fixed.pars.idx=fixed.pars.idx, 
                   control=list(maxit=1e6,reltol=1e-5),method=method,...)
      opt$fW.par = opt$par
    } else {
      stop("ERROR in likelihood setup")
    }
    mle[-fixed.pars.idx] = opt$par
    # i=i+1
  }
  return(list(mle=mle,
              fixed.pars.idx=fixed.pars.idx))
}

