...
##############################################################################
##############################################################################
# d=3, but this can easily be generalized later to d>2

iweights.3d.pwl = function (k, r0w, w, gfun, shape=3, par){
  # WARNING: assumed fixed shape parameter of 3
  if (k < 1) {
    warning("k is below 1, adjusted to k = 1.")
    k = 1
  }
...
sim.3d.cond = function (w, r0w, k = 1, nsim, shape=3, par, gfun) {
  # WARNING: assumed fixed shape parameter of 3
  if (k != 1) {
    iw <- iweights.3d.pwl(k = k, r0w = r0w, w = w, gfun = gfun, par = par)
...
fW.mcmc.g.3d<-function(niter,nburn,alpha=rep(1,3),thin,g,return.acc.rate=FALSE,...){
  ...
  while(it<=niter){
    ...
    accn<-LaplacesDemon::ddirichlet(w,alpha)*(g(wcan,...)^(-3))
    accd<-LaplacesDemon::ddirichlet(wcan,alpha)*(g(w,...)^(-3))
...
sim.3d.joint = function(nsim,k.vals=1,gfun,shape=3,par,fW.par,par.locs,par.locs.W=NULL,
  ...
  nthin=3
  wstar = fW.mcmc.g.3d(niter=(nthin*nsim)+1000,nburn=1000,thin=nthin,
                       alpha=alpha,
                       g=gfun,par=fW.par,locs=par.locs.W)
  
  # now, evaluate r0w at the sampled angles
  r.tau.wstar = apply(wstar,1,function(wstar.i){
    weightsw<-dmvnorm(w[,1:2],mean=as.numeric(wstar.i[1:2]),sigma=bww^2*diag(2))
    ccdf<-function(rc){
      mean(weightsw*pnorm(rc,mean=r,sd=bwr))/mean(weightsw)
    }
    dummy<-function(rc){ccdf(rc) - tau}  # want the root of this
  ...
  sims = lapply(k.vals, function(k){
    if(k==1){
      ...
      iw <- iweights.3d.pwl(k=k,r0w=r.tau.wstar,w=wstar,gfun=gfun,shape=shape,par=par)
...
sim.cond = function (w, r0w, k = 1, nsim, shape, par, gfun) {
...
sim.joint = function(nsim,k.vals=1,shape,par,fW.par,par.locs,par.locs.W=NULL,
                     r,w,tau=0.95,bww=0.05,bwr=0.05){
