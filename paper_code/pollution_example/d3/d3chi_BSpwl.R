...
for(rep in c(start.idx:num.bs.reps)){
  ...
  if(skip_to_next) {
    ...
    # chi estimation
    pwl.res1 = pwl.res2 = pwl.res3 = pwl.res4 = pwl.res5 = pwl.res6 = list(list(NULL))
    
    col.idx=1
    for(which.cols in idx.groups){
      message(which.cols)
      u.vals = u.vals.lst[[col.idx]]
      # pwl.res1[[col.idx]] = mean(excind.bs,na.rm=T)*sapply(u.vals, function(u) mean(apply(xstar.pwl.bs.1[,which.cols],1,function(xx){all(pexp(xx)>u)}))/(1-u))
      # pwl.res2[[col.idx]] = mean(excind.bs,na.rm=T)*sapply(u.vals, function(u) mean(apply(xstar.pwl.bs.2[,which.cols],1,function(xx){all(pexp(xx)>u)}))/(1-u))
      # pwl.res3[[col.idx]] = mean(excind.bs,na.rm=T)*sapply(u.vals, function(u) mean(apply(xstar.pwl.bs.3[,which.cols],1,function(xx){all(pexp(xx)>u)}))/(1-u))
      pwl.res4[[col.idx]] = mean(excind.bs,na.rm=T)*sapply(u.vals, function(u) mean(apply(xstar.pwl.bs.4[,which.cols],1,function(xx){all(pexp(xx)>u)}))/(1-u))
      # pwl.res5[[col.idx]] = mean(excind.bs,na.rm=T)*sapply(u.vals, function(u) mean(apply(xstar.pwl.bs.5[,which.cols],1,function(xx){all(pexp(xx)>u)}))/(1-u))
      # pwl.res6[[col.idx]] = mean(excind.bs,na.rm=T)*sapply(u.vals, function(u) mean(apply(xstar.pwl.bs.6[,which.cols],1,function(xx){all(pexp(xx)>u)}))/(1-u))
      col.idx = col.idx + 1
    }
    
    bs.res.pwl[[rep]] = list(pwl.bs.pars=list(
                                          W.fit.34.bs=W.fit.34.bs$fW.mle,
                                          # R.fit.13.bs=R.fit.13.bs$mle,
                                          R.fit.24.bs=R.fit.24.bs$mle#,
                                          # RW.fit.5.bs=RW.fit.5.bs$mle,
                                          # RW.fit.6.bs=RW.fit.6.bs$mle
                                          ),
                         pwl.res=list(
                                      # pwl.res1=pwl.res1,
                                      # pwl.res2=pwl.res2
