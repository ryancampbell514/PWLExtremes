...
plotfittedthresh(qr)
htmlwidgets::saveWidget(rglwidget(),file="diagnostics/d3pollution_thresh.html")
...
message("fitting R")
R.fit = fit.geometric.pwl(r=rexc,r0w=r0wexc,w=wexc,locs=par.locs,init.val=init,
                          fixshape=T,pen.const=1,bound.fit=T)
xstar.sims.cond = PWLExtremes::sim.cond(w=wexc,r0w=r0w,nsim=50000,k=1,
                                        ...
                                        par=R.fit$mle)
base::save.image(file="d3pollution_fit_akdethresh.RData")

# message("Fitting W")
# W.fit = fit.geometric.pwl(r=rexc,r0w=r0w,w=wexc,locs=par.locs,init.val=init,
#                           fixshape=T,pen.const=20, #0.137931034482759,
...
# 
# save.image(file="d3pollution_fit_akdethresh.RData")
# stop()
...
plotfittedthresh(thresh.fit=qr)
htmlwidgets::saveWidget(rglwidget(),file="diagnostics/d3pollution_thresh.html")
...
mle =  R.fit$mle#pen.fit.res[[1]]$RW.fit.5$mle
wpts = seq(0,1,length.out=100)
wpts = expand.grid(wpts,wpts)
wpts = cbind(wpts,1-apply(wpts,1,sum))
cond = wpts[,3]<0
wpts[cond,]=NA
g.vals = gfun.pwl(x=wpts,par=mle, ref.angles=par.locs)
plot3d(ds.exp.3d/log(nrow(ds.exp.3d)))
...
axes3d()
title3d(xlab=colnames(ds.exp.3d)[1],ylab=colnames(ds.exp.3d)[2],zlab=colnames(ds.exp.3d)[3])
surface3d(x=wpts[,1]/matrix(g.vals,nrow=sqrt(length(g.vals)),ncol=sqrt(length(g.vals))),
          y=wpts[,2]/matrix(g.vals,nrow=sqrt(length(g.vals)),ncol=sqrt(length(g.vals))),
          z=wpts[,3]/matrix(g.vals,nrow=sqrt(length(g.vals)),ncol=sqrt(length(g.vals))),
          col="blue",alpha=0.4)
view3d(userMatrix = usermat,zoom=0.8)
htmlwidgets::saveWidget(rglwidget(),file="diagnostics/d3pollution_g_est.html")
# snapshot3d("diagnostics/d3pollution_g_est.png",width=500,height=500)
...
pdf("diagnostics/d3pollution_PP.pdf",width=5,height=5)
...
abline(a = 0, b = 1)
dev.off()
pdf("diagnostics/d3pollution_QQ.pdf",width=5,height=5)
par(mfrow=c(1,1),pty="s",mar=c(4.2,4.2,4.2,4.2),cex.lab=1.5)
...
source("../../../../geometricMVEdev/thresh_estimation.R")
w.mesh = wpts
thresh.r = eval.thresh(qr,w.mesh)
...
load("../fitting_orig/u_vals_d3pollution.RData")

# chi.vals.emp = list(list(NULL))
xstar.sims = xstar.sims.cond#sim.cond(w=wexc,r0w=r0wexc,nsim=50000,k=1,gfun=gfun,par=R.fit.24$mle)
...
for(which.cols in idx.groups){
  ...
  pdf(paste0("diagnostics/d3pollution_chi_plot_",name,".pdf"),height=5,width=5)
  par(mfrow=c(1,1),pty="s",mar=c(4,4.5,1,4.5),cex.axis=1.5,cex.lab=1.5)
  plot(u.vals,chi.u.emp,type="l",lwd=1,
       xlab="u",ylab=ylab,
       ylim=c(0,1.0))
...
# Return levels
gauge.est.vals.w = gfun.pwl(w,par=R.fit$mle,ref.angles=par.locs)
...
pdf("diagnostics/d3pollution_return.pdf",width=5,height=5)
par(mfrow=c(1,1),pty="s",mar=c(4.2,4.2,4.2,4.2),cex.lab=1.5)
plot(NULL,
