...
w.mesh = qr$wpts
# w.mesh = seq(0,1,length.out=30)
# w.mesh = expand.grid(w.mesh,w.mesh,w.mesh)
...
for(which.cols in idx.groups){
  ...
  # chi.u.est.pwl = (1-tau)*sapply(u.vals, function(u) mean(apply(xstar.sims[,which.cols],1,function(xx){all(pexp(xx)>u)}))/(1-u))
  ...
  pdf(paste0("d4pollution_chi_plot_",name,".pdf"),height=5,width=5)
