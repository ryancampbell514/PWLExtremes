...
bww = pdfCluster::kepdf(x = thr$w, kernel = "gaussian", bwtype="adaptive",
                        alpha=0.5)@par$hx
...
rtilde.proj<-function(x,which.min,upper=10,res=100)
{
  ...
  if(which.min==1){
    ...
    {
      w.inp = c(y,x)
      sum(w.inp) / eval.thresh(thr,w.eval = matrix(w.inp/sum(w.inp),nrow=1),bww=bww)
    }
  } else if(which.min==2){
    dummy<-function(y)
    {
...
open3d()
plot3d(ds.exp.4d[,-1],col="black",alpha=1)
...
open3d()
plot3d(ds.exp.4d[,-2],col="black",alpha=1)
...
open3d()
plot3d(ds.exp.4d[,-3],col="black",alpha=1)
...
open3d()
plot3d(ds.exp.4d[,-4],col="black",alpha=1)
