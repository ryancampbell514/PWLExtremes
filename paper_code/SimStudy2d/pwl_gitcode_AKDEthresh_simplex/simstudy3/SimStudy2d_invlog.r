...
inv.logistic.cdf <- function(x.vec,dep.par){
...
pwl.prob1.est.geom<-pwl.prob1.est.geom2<-pwl.prob1.est.geom3<-rep(NA,nrep)
pwl.prob2.est.geom<-pwl.prob2.est.geom2<-rep(NA,nrep)
pwl.prob3.est.geom<-pwl.prob3.est.geom2<-rep(NA,nrep)
...
load("../../quantiles/quantiles_kde/invlog_quantiles.Rdata")
...
for(k in 1:nrep){
  ...
  skip_to_next <- FALSE
  tryCatch({
    fW.fit = fit.geometric.pwl(r=rexc,r0w=r0w,w=wexc,locs=par.locs,pen.const=pen.const.W,bound.fit=F,fixshape=T,W.fit=T)
    model.fit = fit.geometric.pwl(r=rexc,r0w=r0w,w=wexc,locs=par.locs,pen.const=pen.const,bound.fit=F,fixshape=T)
  ...
  fW.pars[k,]=fW.fit$mle.W
  nll.vals[k] = model.fit$nll + fW.fit$nll
  ...
  pr.k.val = mean(pgamma(k.vals[3]*r0w, shape = model.fit$shape, rate = sapply(wexc, gfun, par = mle), lower.tail = F)/
                    pgamma(r0w, shape = model.fit$shape, rate = sapply(wexc, gfun, par = mle), lower.tail = F))
  xstar<-xstar.sims[[3]]
  pwl.prob1.est.geom3[k]<-mean(xstar[,1]>x11&xstar[,1]<x12&xstar[,2]>y11&xstar[,2]<y12) * pr.k.val* length(rexc)/length(r)
  
  #########
  if(k%%20==0){save.image("pwlinexp_SimStudy2d_invlog.Rdata")}
  # save.image("pwlinexp_SimStudy2d_invlog.Rdata")
}
