...
prob3.est.geomE3<-prob3.est.geomS3<-NULL
...
for(k in 1:nrep){
  ...
  set.seed(k)
  x<-rbvevd(n,dep=aAD,mar1=c(0,1,0))
  ...
  prob3.est.geomS3[k]<-mean(xstar[,1]>x31&xstar[,1]<x32&xstar[,2]>y31&xstar[,2]<y32) * Rexc.prob.k.2d(k=ks, r0w=r0w, w=wexc, gfun=fittedgfun, par=fittedpar)* length(rexc)/length(r)
  ...
  prob3.est.geomE3[k]<-mean(xstar2[,1]>x31&xstar2[,1]<x32&xstar2[,2]>y31&xstar2[,2]<y32) * Rexc.prob.k.2d(k=ks, r0w=r0w2, w=wexc2, gfun=fittedgfun, par=fittedpar)* length(rexc2)/length(r)
  ...
  # # Do this...
  # aics = unlist(optim.res["aic",])
  # ord = 2
  # while(M==Inf){
  #   best.h.idx = which(order(aics,decreasing=T)==ord)
  #   best.h = spec.dens.fns[[best.h.idx]]
  #   best.H = H.fns[[best.h.idx]]
  #   h.mle = optim.res["par",][[best.h.idx]]
  #   M = max(best.h(w = w.mesh, dep = h.mle)/
  ...
  while(idx<=n1){
    ...
    if(prop.distn=="beta"){
      ...
      dprop = dbeta(prop,0.5,0.5)
    } else {
      prop = runif(1)
      dprop = dunif(prop)
    }
    acc.rej.cond = (best.h(w = prop, dep = h.mle)) / (M*dprop)
    if(u < acc.rej.cond){
      wstar[idx] = prop
      idx = idx + 1
    }
...
#save.image("~/Dropbox/phd_research/geomMVE_work/r_code/SimStudy2d_code/SimStudy2d_log_weakdep.Rdata")

#load("SimStudy2d_log_weakdep.Rdata")
