...
for(k in 1:nrep){
  ...
  while(idx<=n1){
    u = runif(1)
    prop = runif(1)
    acc.rej.cond = (best.h(w = prop, dep = h.mle)) / (M*dunif(prop))
    if(u < acc.rej.cond){
      wstar[idx] = prop
      idx = idx + 1
    }
...
#save.image("~/Dropbox/phd_research/geomMVE_work/r_code/SimStudy2d_code/SimStudy2d_log_strongdep.Rdata")

# load("SimStudy2d_log_strongdep.Rdata")
