...
load("~/Dropbox/phd_research/pw_lin_gauge/SimStudy_2d/quantiles/quantiles_kde/gauss_quantiles.Rdata")
...
for(k in 1:nrep){
  ...
  fit = fit.geometricMVE.2d.fixedshape(r=rexc,w=wexc,r0w=r0w,gfun=gauge_gaussian,
                                       init.val=0.5,fixed.shape=T,pos.par=T)
  pars[k,]<-fit$mle
  llvals[k]<-fit$nllh
  
  xstar<-sim.2d(w=wexc,r0w = r0w,nsim = n1,par=fit$mle,gfun=gauge_gaussian)
  ...
  xstar<-sim.2d(w=wexc,r0w = r0w,nsim = n1,par=fit$mle,gfun=gauge_gaussian,k=km)
  
  prob1.est.geom2[k]<-mean(xstar[,1]>x11&xstar[,1]<x12&xstar[,2]>y11&xstar[,2]<y12) * Rexc.prob.k.2d(k=km, r0w=r0w, w=wexc, gfun=gauge_gaussian, par=fit$mle)* length(rexc)/length(r)
  prob2.est.geom2[k]<-mean(xstar[,1]>x21&xstar[,1]<x22&xstar[,2]>y21&xstar[,2]<y22) * Rexc.prob.k.2d(k=km, r0w=r0w, w=wexc, gfun=gauge_gaussian, par=fit$mle)* length(rexc)/length(r)
  prob3.est.geom2[k]<-mean(xstar[,1]>x31&xstar[,1]<x32&xstar[,2]>y31&xstar[,2]<y32) * Rexc.prob.k.2d(k=km, r0w=r0w, w=wexc, gfun=gauge_gaussian, par=fit$mle)* length(rexc)/length(r)
  ...
  xstar<-sim.2d(w=wexc,r0w = r0w,nsim = n1,par=fit$mle,gfun=gauge_gaussian,k=ks)
  prob3.est.geom3[k]<-mean(xstar[,1]>x31&xstar[,1]<x32&xstar[,2]>y31&xstar[,2]<y32) * Rexc.prob.k.2d(k=ks, r0w=r0w, w=wexc, gfun=gauge_gaussian, par=fit$mle)* length(rexc)/length(r)
  
  print(k)
  if(k%%20==0){save.image("SimStudy2d_gauss.Rdata")}
}
rm(gauss.res)
time.end<-Sys.time()
save.image("SimStudy2d_gauss.Rdata")
