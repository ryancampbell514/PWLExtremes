...
load("~/Dropbox/phd_research/pw_lin_gauge/SimStudy_2d/quantiles/quantiles_kde/log_weakdep_quantiles.Rdata")
...
for(k in 1:nrep){
  ...
  if(k%%20==0){save.image("SimStudy2d_log_weakdep.Rdata")}
}
rm(log.weakdep.res)
time.end<-Sys.time()
save.image("SimStudy2d_log_weakdep.Rdata")
