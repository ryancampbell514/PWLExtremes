...
pwl.prob1.est.geom<-pwl.prob2.est.geom<-pwl.prob3.est.geom<-numeric(nrep)
pwl.prob1.est.geom2<-pwl.prob2.est.geom2<-pwl.prob3.est.geom2<-numeric(nrep)
pwl.prob1.est.geom3<-pwl.prob2.est.geom3<-pwl.prob3.est.geom3<-numeric(nrep)
...
load("~/lancaster/pw_lin_gauge/SimStudy_3d/quantiles/quantiles_kde/alog2_quantiles.Rdata")
...
for(k in 1:nrep){
  ...
  # surface3d(x=qr$wpts[,1] * qr$r.tau.wpts,
  #           y=qr$wpts[,2] * qr$r.tau.wpts,
  #           z=qr$wpts[,3] * qr$r.tau.wpts,
  #           col="red",alpha=0.5)
  # axes3d()
  ...
  tryCatch({
    ...
    model.fit = fit.geometric.pwl(r=rexc,w=wexc,r0w=r0w,locs=par.locs,pen.const=1,bound.fit=T)
  },error=function(e){
    skip_to_next <<- TRUE
  })
  if(skip_to_next) {
  ...
  ###################################
  if(k%%20==0){save.image("pwlinexp_SimStudy3d_alog2.Rdata")}
}
time.end<-Sys.time()
rm(alog2.res)
save.image("pwlinexp_SimStudy3d_alog2.Rdata")
