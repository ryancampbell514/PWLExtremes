...
# Conditional extremes and tau calculations + simulation from copula:
...
source("Extra Functions/InvClaytonInvLogisticFunctions.r")
...
p1<-Joint.cdf(c(x12,y12,z12), alpha=0.5,beta=2)
p2<-Joint.cdf(c(x11,y12,z12), alpha=0.5,beta=2)
p3<-Joint.cdf(c(x12,y11,z12), alpha=0.5,beta=2)
p4<-Joint.cdf(c(x12,y12,z11), alpha=0.5,beta=2)
p5<-Joint.cdf(c(x11,y11,z12), alpha=0.5,beta=2)
p6<-Joint.cdf(c(x11,y12,z11), alpha=0.5,beta=2)
p7<-Joint.cdf(c(x12,y11,z11), alpha=0.5,beta=2)
p8<-Joint.cdf(c(x11,y11,z11), alpha=0.5,beta=2)

TP1<-p1-p2-p3-p4+p5+p6+p7-p8

p1<-Joint.cdf(c(x22,y22,z22), alpha=0.5,beta=2)
p2<-Joint.cdf(c(x21,y22,z22), alpha=0.5,beta=2)
p3<-Joint.cdf(c(x22,y21,z22), alpha=0.5,beta=2)
p4<-Joint.cdf(c(x22,y22,z21), alpha=0.5,beta=2)
p5<-Joint.cdf(c(x21,y21,z22), alpha=0.5,beta=2)
p6<-Joint.cdf(c(x21,y22,z21), alpha=0.5,beta=2)
p7<-Joint.cdf(c(x22,y21,z21), alpha=0.5,beta=2)
p8<-Joint.cdf(c(x21,y21,z21), alpha=0.5,beta=2)

TP2<-p1-p2-p3-p4+p5+p6+p7-p8

p1<-Joint.cdf(c(x32,y32,z32), alpha=0.5,beta=2)
p2<-Joint.cdf(c(x31,y32,z32), alpha=0.5,beta=2)
p3<-Joint.cdf(c(x32,y31,z32), alpha=0.5,beta=2)
p4<-Joint.cdf(c(x32,y32,z31), alpha=0.5,beta=2)
p5<-Joint.cdf(c(x31,y31,z32), alpha=0.5,beta=2)
p6<-Joint.cdf(c(x31,y32,z31), alpha=0.5,beta=2)
p7<-Joint.cdf(c(x32,y31,z31), alpha=0.5,beta=2)
p8<-Joint.cdf(c(x31,y31,z31), alpha=0.5,beta=2)
...
for(k in 1:nrep){
  ...
  ######## GEOMETRIC ################
  clay.cop <- claytonCopula(2)
  uv <- rCopula(5000, clay.cop)
  uv<-cbind(1-uv[,1],1-uv[,2])
  xy<--log(1-uv)
  U<-runif(5000)
  X3<-condsim.ievl(xy[,2],0.5,U)
  x<-cbind(xy,X3)
  ####
  ...
  llvals[k,]<-c(f1$nllh,f2$nllh,f3$nllh)
  AICvals[k,]<-c(2*f1$nllh+2*(1+length(ind1[ind1>3])),2*f2$nllh+2*(1+length(ind2[ind2>3])),2*f3$nllh+2*(1+length(ind3[ind3>3])))
  ...
  ###################################
  if(k%%20==0){save.image("SimStudy3d_invclayinvlog.Rdata")}
}
time.end<-Sys.time()

save.image("SimStudy3d_invclayinvlog.Rdata")
