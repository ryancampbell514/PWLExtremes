...
# setwd("~/Dropbox/phd_research/pw_lin_gauge/SimStudy_3d/pwl_gradpen_finermesh_betterlambda/simstudy1/")
...
load("pwlinexp_SimStudy3d_alog2.Rdata")
start.k = k
time.start<-Sys.time()
# set.seed(101)
for(k in start.k:nrep){
  ...
  skip_to_next <- FALSE
  tryCatch({
    model.fit = fit.geometric.pwl(r=rexc,w=wexc,r0w=r0w,locs=par.locs,pen.const=1,joint.fit=T,bound.fit=T)
  },error=function(e){
    skip_to_next <<- TRUE 
  })
  if(skip_to_next) {
