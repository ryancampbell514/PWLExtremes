...
for(k in start.k:nrep){
  ...
  ###################################
  if(k%%20==0){save.image("pwlinexp_SimStudy3d_mix.Rdata")}
}
time.end<-Sys.time()
rm(mix.res)
save.image("pwlinexp_SimStudy3d_mix.Rdata")
